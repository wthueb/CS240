...
static const Point points[] = {
  Point(0, 0), Point(3, 4), Point(1, 1), Point(4, 5), Point(9,9)
};
...
int main() {
  ...
  for (auto i = 0ul; i < nPoints - 1; i+=2)
  {
      auto line = Line(points[i], points[i+1]);

      out << line.toString() << " length = " << line.length() << std::endl;
  }
}
